...
C     viscA?_W  :: Horizontal variable viscosities
     ...
     &                   phi_nh, dPhiNH,
     &                   gW, gwNm1
      Real*8  phi_nh(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  dPhiNH(1-OLx:sNx+OLx,1-OLy:sNy+OLy,   nSx,nSy)
      Real*8  gW   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  gwNm1(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
...
C     i,j,k         :: Loop counters
      ...
      Real*8    flx_NS(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8    flx_EW(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8    flx_Dn(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8    flxAdvUp(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8    flxDisUp(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8    gwDiss(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8    gwAdd (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8    gw_AB (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8    del2w (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8    wFld  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      INTEGER i,j,k, km1, kp1
      Real*8  mskM1, mskP1
      Real*8  tmp_WbarZ
      Real*8  uTrans, vTrans, rTrans
      Real*8  viscLoc
...
C     W-Cell Western face area:
           xA(i,j) = dyG(i,j,bi,bj)*rThickC_W(i,j)
c    &                              *deepFacF(k)
          ENDDO
         ENDDO
...
C     W-Cell Southern face area:
           yA(i,j) = dxG(i,j,bi,bj)*rThickC_S(i,j)
c    &                              *deepFacF(k)
...
c          xA(i,j) = dyG(i,j,bi,bj)*drC(k)
c          yA(i,j) = dxG(i,j,bi,bj)*drC(k)
          ENDDO
         ENDDO
        ENDIF
...
C     transport through Western face area:
             uTrans = (
     &          drF(km1)*hFacW(i,j,km1,bi,bj)*uVel(i,j,km1,bi,bj)
     &                  *rhoFacC(km1)*mskM1
     &        + drF( k )*hFacW(i,j, k ,bi,bj)*uVel(i,j, k ,bi,bj)
     &                  *rhoFacC(k)
     &                )*halfRL*dyG(i,j,bi,bj)*deepFacF(k)
...
C     transport through Southern face area:
             vTrans = (
     &          drF(km1)*hFacS(i,j,km1,bi,bj)*vVel(i,j,km1,bi,bj)
     &                  *rhoFacC(km1)*mskM1
     &         +drF( k )*hFacS(i,j, k ,bi,bj)*vVel(i,j, k ,bi,bj)
     &                  *rhoFacC(k)
     &                )*halfRL*dxG(i,j,bi,bj)*deepFacF(k)

...
C  myThid         :: my Thread Id number
      Real*8 gTendency(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      INTEGER k, bi, bj
      INTEGER tracerNum
      Real*8 myTime
      INTEGER myIter
...
C  i,j            :: loop indices
      INTEGER i,j
      INTEGER irbc
      Real*8 rbcsVanishingFac
      Real*8 rec_tauRlx
      INTEGER iTracer
CEOP


      IF ( rbcsVanishingTime.GT.0.D0 ) THEN
        rbcsVanishingFac =
     &      MAX( 0.D0 , 1.D0 - myTime / rbcsVanishingTime )
      ELSE
        rbcsVanishingFac = 1.D0
      ENDIF

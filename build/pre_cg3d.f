...
c     CHARACTER*(MAX_LEN_MBUF) msgBuf
      Real*8     locGamma, surfFac, tmpFac
      Real*8     wFacKm, wFacKp
      Real*8     uf(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     vf(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     wSurfP2d(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
CEOP
...
C      (has been done for cg2d_b ; and addMass was added by CALC_DIV_GHAT)
     ...
     &          + tmpFac*rA(i,j,bi,bj)*EmPmR(i,j,bi,bj)
     &                                 *maskInC(i,j,bi,bj)
            ENDDO
           ENDDO
          ELSE
...
C-       Other than Z coordinate: no assumption on surface level index
     ...
     &                  *rA(i,j,bi,bj)*deepFac2F(ks)/deltaTMom
              ENDIF
            ENDDO
           ENDDO
         ENDIF
...
C                             2) add vertical velocity contribution.
         DO j=1,sNy+1
          DO i=1,sNx+1
           uf(i,j) = -recip_dxC(i,j,bi,bj)
     &             * implicSurfPress*implicDiv2DFlow
     &             *(cg2d_x(i,j,bi,bj)-cg2d_x(i-1,j,bi,bj))
     &             *maskInC(i,j,bi,bj)*maskInC(i-1,j,bi,bj)
           vf(i,j) = -recip_dyC(i,j,bi,bj)
     &             * implicSurfPress*implicDiv2DFlow

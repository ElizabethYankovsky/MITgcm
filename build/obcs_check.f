...
C     Eastern boundary
          IF ( tileHasOBE(bi,bj) .AND. OBCS_balanceFacE .GT. 0.D0 )
     &         nTileWithActiveOB = nTileWithActiveOB + 1
C     Western boundary
          IF ( tileHasOBW(bi,bj) .AND. OBCS_balanceFacW .GT. 0.D0 )
     &         nTileWithActiveOB = nTileWithActiveOB + 1
C     Northern boundary
          IF ( tileHasOBN(bi,bj) .AND. OBCS_balanceFacN .GT. 0.D0 )
     &         nTileWithActiveOB = nTileWithActiveOB + 1
C     Southern boundary
          IF ( tileHasOBS(bi,bj) .AND. OBCS_balanceFacS .GT. 0.D0 )
     &         nTileWithActiveOB = nTileWithActiveOB + 1
         ENDDO
        ENDDO

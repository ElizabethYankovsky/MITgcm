...
C  heightStepN          :: height of topographic step to North
      INTEGER i,j
      Real*8 heightStepW, heightStepE
      Real*8 heightStepS, heightStepN
CEOP
...
C       (but this might just be what we want ...)
        heightStepW = MAX( 0.D0, rThickC(i,j) - rThickC(i-1,j) )
        heightStepE = MAX( 0.D0, rThickC(i,j) - rThickC(i+1,j) )
        heightStepS = MAX( 0.D0, rThickC(i,j) - rThickC(i,j-1) )
        heightStepN = MAX( 0.D0, rThickC(i,j) - rThickC(i,j+1) )
C- jmc: take directly viscAh_W & viscA4_W (no bar_i, bar_j) since
C       they are already at the center (and not above u,v point).
        gwSideDrag(i,j) = 
     &   -sideDragFactor
     &    *( heightStepW*dyG( i ,j,bi,bj)*recip_dxC( i ,j,bi,bj)
     &         *( viscAh_W(i,j,k,bi,bj)*wFld(i,j,k,bi,bj)
     &                                 *cosFacV(j,bi,bj)
     &           -viscA4_W(i,j,k,bi,bj)*del2w(i,j)
     &                                 *cosFacU(j,bi,bj)
     &          )
     &     +heightStepE*dyG(i+1,j,bi,bj)*recip_dxC(i+1,j,bi,bj)
     &         *( viscAh_W(i,j,k,bi,bj)*wFld(i,j,k,bi,bj)
     &                                 *cosFacV(j,bi,bj)
     &           -viscA4_W(i,j,k,bi,bj)*del2w(i,j)
     &                                 *cosFacU(j,bi,bj)
     &          )
     &     +heightStepS*dxG(i,j,bi,bj)*recip_dyC(i,j,bi,bj)
     &         *( viscAh_W(i,j,k,bi,bj)*wFld(i,j,k,bi,bj)
     &           -viscA4_W(i,j,k,bi,bj)*del2w(i,j)
     &          )
     &     +heightStepN*dxG(i,j+1,bi,bj)*recip_dyC(i,j+1,bi,bj)
     &         *( viscAh_W(i,j,k,bi,bj)*wFld(i,j,k,bi,bj)
     &           -viscA4_W(i,j,k,bi,bj)*del2w(i,j)
     &          )

...
C     cg3dNorm - A matrix normalisation factor.
     ...
     &      zMC, zML, zMU,
     &      cg3dNorm
      Real*8  aW3d (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  aS3d (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  aV3d (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  aC3d (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  zMC  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  zML  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  zMU  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  cg3dNorm
...
C     cg3d_s -   idem
     ...
     & cg3d_q, cg3d_r, cg3d_s
      Real*8  cg3d_q(1-1  :sNx+1  ,1-1  :sNy+1  ,Nr,nSx,nSy)
      Real*8  cg3d_r(1-1  :sNx+1  ,1-1  :sNy+1  ,Nr,nSx,nSy)
      Real*8  cg3d_s(1-1  :sNx+1  ,1-1  :sNy+1  ,Nr,nSx,nSy)
...
C     cg3d_b  :: Right Hand side vector of the 3-D solver equation A.x=B
     ...
     &                        cg3d_b
c    &                        cg3d_x, cg3d_b
c     Real*8  cg3d_x(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nR,nSx,nSy)
...
C     myNorm   :: Work variable used in clculating normalisation factor
      ...
      Real*8     myNorm
      Real*8     theRecip_Dr
      Real*8     aU, aL, aW, aE, aN, aS
      Real*8     tmpFac, nh_Fac, igwFac
      Real*8     locGamma
CEOP

CcnhDebugStarts
c     Real*8    phi(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
CcnhDebugEnds
...
C-       From common bloc SFP3D_COMMON_R:
      ...
      igwFac = 0.
      IF ( nonHydrostatic
     &      .AND. nh_Am2.NE.0. ) nh_Fac = 1.D0 / nh_Am2
      IF ( implicitIntGravWave ) igwFac = 1.D0
...
C  deep-model: *deepFacC (faceArea), /deepFacC (recip_dx,y): => no net effect
     ...
     &          + igwFac*dBdrRef(k)*deltaTMom*dTtracerLev(k)
         IF (tmpFac.GT.0. ) tmpFac = 1.D0 / tmpFac
         DO j=1,sNy
          DO i=1,sNx
           faceArea = rA(i,j,bi,bj)*maskC(i,j, k ,bi,bj)
     &                              *maskC(i,j,k-1,bi,bj)
     &                              *deepFac2F(k)
     &                              *maskInC(i,j,bi,bj)
           theRecip_Dr = recip_drC(k)
c          theRecip_Dr =
caja &      drF(k  )*hFacC(i,j,k  ,bi,bj)*0.5
caja &     +drF(k-1)*hFacC(i,j,k-1,bi,bj)*0.5
c          IF ( theRecip_Dr .NE. 0. )
c    &      theRecip_Dr = 1.D0/theRecip_Dr
...
C--   Update overlap regions
      ...
      CALL EXCH_XYZ_RS ( aV3d,  myThid )
      CALL EXCH_XYZ_RS ( aC3d,  myThid )
CcnhDebugStarts
...
CcnhDebugEnds
        ...
        k = 1
         DO j=1,sNy
          DO i=1,sNx
           zMC(i,j,k,bi,bj) = 1.D0 / zMC(i,j,k,bi,bj)
...
CcnhDEbugEnds
      ...
      ENDDO
C--   Update overlap regions
      CALL EXCH_XYZ_RS ( zMC,  myThid )
      CALL EXCH_XYZ_RS ( zML,  myThid )
      CALL EXCH_XYZ_RS ( zMU,  myThid )

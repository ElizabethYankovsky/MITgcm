...
C     myThid   :: my Thread Id number
      ...
      INTEGER mSize
      INTEGER gPos
      Real*8 arr    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,kSize,nSx,nSy)
      Real*8 arrhFac(1-OLx:sNx+OLx,1-OLy:sNy+OLy,mSize,nSx,nSy)
      Real*8 arrDy  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 arrDr  (kSize)
      Real*8 mskInC (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      INTEGER myThid

C     !OUTPUT PARAMETERS:
C     arrStats :: field statistics at Eatern & Western OB
      Real*8 arrStats(0:4,2)
CEOP
...
C     ii, iB   :: local index of open boundary
      ...
      INTEGER ii, iB
      LOGICAL noPnts
      Real*8 tmpA, tmpV, tmpMask
      Real*8 theMin, theMax, theArea, theMean, theVar
...
C     myThid   :: my Thread Id number
      ...
      Real*8 arr    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,kSize,nSx,nSy)
      Real*8 arrhFac(1-OLx:sNx+OLx,1-OLy:sNy+OLy,mSize,nSx,nSy)
      Real*8 arrDx  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 arrDr  (kSize)
      Real*8 mskInC (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      INTEGER myThid
